...
function lintLevel(
  ...
  // Compare the order by which template heading each matched section satisfies, not by
  // raw text: a template heading holding a placeholder never equals the document text
  // it legitimately matched, so comparing strings reports every such note as misordered.
  const matchedSections = docSections.filter((section) => consumed.has(section));
  const docOrder = matchedSections.map((section) => {
    const node = templateNodes.find(
      (templateNode) =>
        templateNode.level === section.headingLevel &&
        headingTextMatches(templateNode.text, section.headingText)
    );
    return (node?.text ?? section.headingText).toLowerCase();
  });
